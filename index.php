...
class Site
{
  ...
  public function check()
  {
    ...
    foreach ($this->sData->urls as $key => $u) {	
		...
		if ($pos === false) {
		    $this->sData->urls[$key]->status="FALSE";
		} else {
		    $this->sData->urls[$key]->status="TRUE";
		}
		...
		foreach ($u->keywords as $kkey => $k) {	
			...
			if ($pos === false) {
			     $this->sData->urls[$key]->keywords[$kkey]->status="FALSE";
			} else {
			    $this->sData->urls[$key]->keywords[$kkey]->status="TRUE";
			}
		}
